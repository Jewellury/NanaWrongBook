...
// 现有返回...
{
  nodes: [{ nodeId, name, layer, tier, status, masteryProb, judgeCriteria, sampleItem, lastEvidence }],
...
**数据来源**：
- `edges`：在 map API 中直接 `prisma.knowledgeEdge.findMany()` 查询全量边，不经过 `KnowledgeGraph`（graph.ts 的边是私有结构，无公开 edges 方法）
- `mainlines`：从 `prisma.mainline.findMany()` 获取主线定义，从 `prisma.nodeMainline.findMany()` 获取节点归属

**详情卡字段**：
- nodes 查询增加 `judgeCriteria`、`sampleItem`、`teachingNotes` 字段（schema 中已有）
- `lastEvidence` 从 StudentNodeState 读取，文案用"最近一次确认是在..."而非"点亮于"（更严谨）
- 响应中 nodes 每项追加 `lastEvidence: string | null`

**mainlineId 筛选时的 edges 过滤**：
当请求携带 `mainlineId` 参数时，edges 必须只返回两端节点都在当前 `nodes` 集合内的边，避免 SVG 画线到不存在的节点：
...
// 或者在 mainlineId 存在时 query 只查对应主线的边：
where: mainlineId ? {
  OR: [
    { source: { mainlines: { some: { mainlineId } } } },
    { target: { mainlines: { some: { mainlineId } } } },
  ]
} : undefined,
```
...
不使用第三方可视化库（零新依赖）。采用**主线列布局**（高保真 mockup `04-knowledge-map.html` 的设计思路）：
...
5. SVG viewBox 宽度 = 列数 * 220 - 40，高度根据最大列长度计算
...
- 节点名称 + 绿色圆点
- 一句话描述（来自 `teachingNotes` 截取前 60 字，或节点名本身的说明）
- 判定标准（来自 `judgeCriteria`）
- 示例题目（来自 `sampleItem`）
- 最近确认日期（来自 `lastEvidence`，文案用"最近一次确认是在...")
- 不显示 masteryProb 百分比（P4）
...
| 节点详情 | "最近一次确认是在 5 月 12 日" | "掌握概率 85%" / "点亮于 5 月 12 日" |
