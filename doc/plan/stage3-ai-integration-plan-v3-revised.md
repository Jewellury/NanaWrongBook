...
> 计划者: plan-agent
> 版本: **v3-revised 收敛版 r2（基于评审 5+4 项反馈修订）**
...
│   ├─ 第一章 集合与常用逻辑用语
│   │   ├─ 1.1 集合的概念（3 道）
│   │   │   └─ [题图缩略图] [AI摘要] [课本分类] [AI反馈] [时间] [有转写标记] [可修正]
│   │   └─ 1.3 集合的基本运算（1 道）
│   │       └─ ...
│   └─ 第三章 函数概念与性质
│       └─ 3.2 函数的基本性质（2 道）
...
| 展示项 | 数据来源 | v1 |
|--------|----------|:--:|
| 题图缩略图 | Artifact(question_image) | ✅ |
| AI 摘要 | CaseAiResult.questionSummary | ✅ |
| 课本分类 | CaseAiResult.textbookTopicId → TextbookTopic.name | ✅ |
| AI 初步反馈 | CaseAiResult.initialFeedback | ✅ |
| 拍摄时间 | Case.createdAt | ✅ |
| 是否有语音转写 | CaseAiResult.transcript 非空 | ✅ |
| 可能的错因提示 | CaseAiResult.possibleMistakeReason | ✅ |
| 下一步建议 | CaseAiResult.nextActionSuggestion | ✅ |
| 手动修正分类 | 点击 → 弹出 TextbookTopic 选择器 | ✅ |
| 手动修正摘要 | 点击 → 编辑 questionSummary | ✅ |
| AI 候选角标 | "AI 候选" / "手动" | ✅ |
...
| 9 | 每题能看到完整信息 | 题目汇总页题卡 | 题图+摘要+分类+反馈+时间+标签 |
...
> 本方案不进入 execute，待评审确认后交 execute-agent 按 Round 0-5 实施。
