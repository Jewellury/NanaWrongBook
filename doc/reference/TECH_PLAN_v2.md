...
### 0.1 产品定位：医生模式（核心隐喻，约束能力边界）★

这个产品像一个一对一的私人医生：
1. **号脉诊断**——找到你不懂的"根源"，而不只是"这道题不会"（对应：KST初诊 + Newman归因 + 探针下探）。
2. **开方转诊**——推荐能打通这个知识点的视频和资料，**它自己不负责把你讲明白**，讲解交给更专业的"药"
   （B站视频、资料）。（对应：节点 videoLinks 推送）
3. **复诊验证**——这是它区别于普通工具的关键：你看完、学完之后，它**再用一道题确认这个点是不是真的通了**；
   没通就换方子或往更深处查。（对应：BKT追踪 + 探针复测）

**这条隐喻带来两个硬性设计约束：**
- **克制边界**：我们不自建讲解内容（讲解是红海，做不过也不该做）。稀缺价值在"诊断"，这是通用AI和现有产品做不好的。
  医生模式让我们心安理得地不做讲解，是一种健康的克制。
- **复诊闭环（防止知识漏洞假性闭合）**：推送视频/资料**绝不是终点**，是一个"需要被验证疗效的中间步骤"。
  药开了必须确认药效——她看完视频后系统出同知识点题，做对该点才从红变绿；做不出说明"药"不对她
  （视频太难/根源更深），换药或继续下探。**"推荐视频"不是一个动作的结束，而是一个验证循环的开始。**

> 对外话术（给用户对她/同学解释用）见运营手册。此处只确立它对系统设计的约束。
...
算法：12-18题，选最高信息量节点出边界题；答对→祖先链全标stable（向下传播），答错→后代链标gap候选。
4-6个uncertain节点各补1道确认题。输出每节点status+BKT先验（stable→0.85/uncertain→0.5/gap→0.15）。
**前台只展示学习前沿（1-2个），其余对她隐藏（P4）**。

#### 4.1.1 首次初诊的特殊设计（沙盘推演逼出的关键修正）★
存在一个内在矛盾：要建地图就得先测，但对一个考30分的孩子，"打开App第一件事是连错5题"本身就是伤害，
违背P4。首次使用几乎决定生死。因此首次初诊必须特殊处理：
- **伪装成"认识你"，不是"测你"**：明确告诉她"这是来看看你现在在哪，不打分、不计错，做不出来直接点
  '还没学'就行"。前台不出现任何"错""失败""得分"字样。
- **从地基题起步，先给安全感**：第一次不从主线边界题开始，从最基础的地基题开始，让她先连对几题建立
  "我还是会一些的"的安全感，再逐步上探。（这与常规初诊"选最高信息量节点"不同，首诊优先心理安全 > 信息效率。）
- **首次使用必须有真人在线陪同**（写进上线SOP）：用户远程在场，不教数学，教操作 + 在她第一次卡壳/做不出时
  即时安抚"没事这就是来看看你在哪"。这个在场系统替代不了。

#### 4.1.2 知识地图的渐进展开（防"一片红"反噬）★
若初诊后地图几十个红点全部铺开，会强化"我数学好烂"的叙事，与"地图即治疗"意图完全相反。所以：
- **首图只点亮"她已经会的"（绿）+ 当前1-2个前沿**，红点暂不画出。
- 红点（全部的洞）作为**她挣来的、后期才解锁的视图**——补掉几个、有了正反馈后再逐步展开。
- 进步以**领先指标**高频可见：点亮的绿点数、新能独立做出的题型，而非滞后的月考分数。
...
## 8.5 数据安全与环境隔离（沙盘推演暴露的工程硬伤，必须落实）★

存在一个结构性矛盾：她在用"线上稳定版"，你在改"开发版"，若共用一套代码和数据库会出事
（开发时改库结构/跑迁移脚本，可能把她两周积累的知识地图搞坏——对她是灾难性打击，可能直接弃用）。
这是纯工程疏忽，但丢一次数据就可能让整个项目归零。硬性要求：

- **生产/开发环境隔离**：她用的是独立部署的稳定生产环境（另一台便宜服务器，或同机不同容器），
  只跑 main 分支的稳定版。你的开发用**测试账号 + 假数据**，绝不在她的真实数据上做开发测试。
- **她的数据每周自动备份**：知识地图、错题、session 记录每周自动备份（单用户，备份成本极低）。
- **任何数据库迁移前先备份**：migrate 前自动快照，出事能回滚。（这条与安全铁律"破坏性操作先确认"呼应。）
- **上线后任何改动遵循**：dev 开发测试 → main 稳定 → 备份 → 部署到她的生产环境，不跳步。
...
| 风险 | 对策 |
|---|---|
| 她不持续使用（头号死因） | P4诊断后台化 + P5纸质化降摩擦 + 地图即治疗 + 共创者身份框架（见运营手册） |
| 用而不投入（机械走流程，数据骗人） | 系统外真人情绪通道，不靠使用数据判断真实效果 |
| 见效慢撑不到正反馈（分数是滞后指标） | 领先指标：点亮绿点数/新掌握题型，高频可见的小进步 |
| 首次使用即挫败 | 首诊伪装成"认识你"、从地基起步、真人在线陪同（4.1.1）；首图只显绿点和前沿（4.1.2） |
| Newman追问触怒青春期孩子 | 追问情绪自适应：识别不耐烦即退让，轮次语气动态调整 |
| LLM归因幻觉 | P3双证据 + Newman prompt"多问少断" + ErrorRecord留日志供抽查 |
| 手写识别不准、信任流失 | P1图为真相源 + 题面人工确认 + 黄金测试集回归 + 输入双通道 |
| 诊断挫败感(ALEKS教训) | P4测评弱可见、只报增量、变式复测不标来源 |
| 推荐视频后撒手（漏洞假性闭合） | 医生模式复诊闭环：推送后必用题验证疗效（0.1） |
| 数据被开发搞坏 | 生产/开发隔离 + 每周备份 + 迁移前必备份（8.5） |
| 教研工作量被低估 | 内容是真瓶颈非代码；MVP敢砍（先函数主线+A层托底即有价值） |
| 用户(你)自己耗尽 | 系统自动化到"她自己能用、你每周轻量介入半小时"；降低对你持续投入的依赖 |
