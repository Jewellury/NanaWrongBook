...
|--------|------|------|
| 诊断 | 整理 / 看看 | OPS §4：术语清零 |
| **AI 初步诊断** | **AI 想对你说 / 可能的方向 / 下一步可以** | **v1 只做轻反馈，不做任何形式的诊断** |
| 已诊断 | 已整理 | |
| 薄弱 | 还没点亮 | 不做负向判断 |
| 得分 | — | 不出现 |
...
**信息架构检查点**：
- "正在整理这题…"对应 `processingStatus=pending`，前端轮询 `/api/nana/cases/:id`，3 秒间隔，最多 60 秒
- **v1 异步整理**：显示"正在整理"时同时显示 [再拍一道] [去题目汇总]，允许用户不等整理完就走
- 超 60 秒提示"整理花的时间有点久，可以稍后在题目汇总里看"，不再轮询
- 整理完成对应 `processingStatus=success`，渲染 `CaseAiResult` 各字段
- 题目汇总页每题卡也显示整理状态：整理中/已整理/整理失败
- **字段名 → 展示文案映射**：
...
- `possibleMistakeReason` 为空时隐藏"可能的方向"区块
- `nextActionSuggestion` 为空时隐藏"下一步"区块
...
- 按课本章节分组排列，每题含：小题图、AI 摘要、AI 想对你说、下一步可以
...
| 题目汇总主卡不展示原图 | 原图懒加载，点"展开看原题"才请求 | ✅ 已确认 |
| 列表 API 不返回 base64 题图 | 原图必须详情懒加载 | ✅ 已确认 |
| 打印页不打印技术元信息 | 时间/置信度/source/转写均不打印 | ✅ 已确认 |
| AI 初步诊断说法清零 | 全局改为"AI 想对你说/可能的方向/下一步可以" | ✅ 已确认 |
| OCR v2 候选 | 先做 Spike，不提前承诺 | ✅ 已确认 |
| 题图缩略图尺寸 | `max-width: 180px; max-height: 120px; object-fit: contain;` | ✅ 已确认 |
| 整理中轮询间隔 | 3 秒起步，最多 60 秒，超时提示"稍后看" | ✅ 已确认 |
| 异步整理选项 | v1 做异步，保存后允许继续拍，汇总显示整理状态 | ✅ 已确认 |
