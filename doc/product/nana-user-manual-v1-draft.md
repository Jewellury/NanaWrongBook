...
### 第 3 步：等 AI 整理（可以不等完，继续拍）

- 点"收好这道题" → 显示"正在收…"（题图存进去了，不会丢）
- 然后显示"正在整理这题…"（AI 在看题图，大约 30 秒）
- **不用干等**——页面同时显示 [再拍一道] [去题目汇总]，可以先拍下一道
- 如果等了超过 1 分钟，会提示"整理花的时间有点久，可以稍后在题目汇总里看"
- 在题目汇总里，正在整理的题会标"整理中…"，整理完了就显示 AI 结果
...
- **页面给了什么反馈**：先显示"正在整理这题…"（约 30 秒），同时显示 [再拍一道] [去题目汇总]——**不用干等**；整理完后展示 AI 摘要、课本分类、鼓励、可能方向、下一步建议；超 1 分钟提示"稍后在题目汇总里看"
...
| 正在整理这题 | AI 在帮忙看题图和录音，不用干等，可以先拍下一道 | 不是"正在诊断" |
| 整理中… | 题目汇总里标记这道题还在整理，稍后回来看结果 | 不是"出问题了" |
