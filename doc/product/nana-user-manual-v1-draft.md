...
Nana 错题本帮孩子做一件事：**把做过的题收起来，AI 帮忙初步整理，按课本章节汇总，可以打印出来复习**。
...
- 每道题显示：题图缩略图、AI 摘要、课本分类、AI 反馈、可能的错因、下一步建议、拍摄时间
- 没有归类的题放在"未分类/暂未覆盖"分组里
...
- 未覆盖的章节：如果拍的题属于当前没覆盖的章节，会归入"未分类/暂未覆盖"
- 后续会随知识图谱扩展追加更多课本章节
...
| 信息 | 展示位置 | 说明 |
|------|---------|------|
| 拍摄时间 | 采集页、题目汇总、打印页 | `Case.createdAt`，即"拍摄于 7月3日" |
| 有语音记录 | 题目汇总题卡 | 如果有录音，显示"有语音记录"图标 |
| 整理时间 | 题目汇总题卡（如有 AI 结果） | `CaseAiResult.updatedAt`，即 AI 最近一次整理时间 |
...
**孩子可能误解什么**：
- ❌ "下一步建议是不是就是解题步骤？"
- ✅ 不是。"下一步可以看看 XX 视频"只是一个建议方向，不是完整解析。AI 不会给你答案。
- ❌ "可能的方向是不是告诉我哪里错了？"
...
### Q："下一步建议"是什么？
"下一步建议"是 AI 给的一个学习方向（比如"可以看看 XX 视频"或"再做几道类似的题"），不是解题步骤，不承诺完整解析。
...
### Q：为什么有些题显示"未分类/暂未覆盖"？
当前课本章节覆盖只到 48 个系统知识点对应的 16 个章节，不是完整教材目录。如果拍的题属于还没覆盖的章节，会归入"未分类/暂未覆盖"。后续会扩展。
