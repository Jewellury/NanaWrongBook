...
- **页面给了什么反馈**：默认停在"题目汇总"tab；按课本章节分组；每道题有缩略图+摘要+分类+时间；有语音的标🎙；未分类的题放最后，带温和提示
- **最容易误解什么**：孩子看到"未分类"可能以为 AI 不会——提示文案"还没放进当前知识地图，先帮你收在这里"不像报错，不怪用户
...
- **页面给了什么反馈**：按课本章节分组排列；每题含题图缩略图+AI 摘要+拍摄时间+转写+轻反馈+可能方向+下一步；未分类放最后；底部有打印和另存 PDF 按钮
