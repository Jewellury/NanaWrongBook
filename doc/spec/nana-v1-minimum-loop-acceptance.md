...
| 维度 | 说明 |
|------|------|
| **用户目标** | 进入 /nana 后默认看到题目汇总，AI 自动分类的题按课本章节分组排列，不需展开就能扫题 |
| **页面反馈** | 汇总 tab 默认选中；AI 分类的题归入对应章节组（chapter + section）；原图默认折叠，点击展开 |
| **API/落库** | `GET /api/nana/cases/summary` → `{ groups: [{ topic, cases }] }`，未分类组排最后 |
| **成功条件** | 多道 AI 分类题正确分入各自章节组；同一章节的题在一起；未分类题在最后；原图不默认展开 |
...
| CL-10a | ✅ | ❌ 待 R1a | — | — | `summary/route.ts` 按 TextbookTopic 分组（AI 自动分类汇总） |
