...
> **状态：第一次冻结完成（FREEZE-001，2026-07-19，source_commit `922045d`）**
> **日期：2026-07-12（初稿）/ 2026-07-19（第一次冻结）**
> **关联：** `doc/plan/nana-test-framework-plan.md`（r3 测试计划）、`doc/product/nana-product-behavior-manual-v1.md`（产品行为手册）、`doc/plan/nana-quality-os-v1-plan.md`（r3.3 Quality OS v1）
> **目的：** 冻结第一版错题集闭环的验收标准。测试框架解决"怎么测"，本文档定义"测什么"和"什么算通过"。
> **冻结记录：** 见 §9 已冻结条款清单（第一次）—— 14 条 R1a 主路径条款已冻结
...
## 9. 已冻结条款清单（第一次）—— Phase A / R1a

> **freeze_id**: FREEZE-001
> **frozen_at**: 2026-07-19
> **approver_event_ref**: A-0 产品复核——用户在 Quality OS v1 Phase A 启动会话中逐条确认
> **source_commit**: `922045d`（契约补丁 commit）
> **scope**: R1a 主路径验收条款
> **frozen_count**: 14 / 17（CL-09、CL-10b、CL-13 不在本次冻结范围）

### 9.1 冻结条款列表（14 条）

| CL | 标题 | 冻结要点 |
|----|------|----------|
| **CL-01** | 一键进入拍题 | 导航+控件出现=硬门禁；≤2s=体验目标（非阻塞）；5s/10s=测试超时 |
| **CL-02** | 题图预览+持久存储 | 原图先压缩（目标约 ≤1MB）；压缩后**总 payload** >3MB 才拒收 |
| **CL-03** | 录音可选 | 不录音也能保存；audioStatus=skipped |
| **CL-04** | 保存不等 AI | 即时"已收好"；≤2s=体验目标；5s/10s=测试超时 |
| **CL-05** | 真实转写与题图解耦 | 音频失败题图正常；真实质量 Provider Smoke 验证 |
| **CL-06** | AI 返回 7 字段（🟡 A-1 补齐） | 7 字段接口契约；轻反馈≠诊断；textbookTopic=null 显示"暂未覆盖"占位（**当前未实现，A-1 补齐**） |
| **CL-07** | 课本+系统两层独立判定 | 两个独立 for 循环；各达阈值各写 tag；0.5 是当前版本策略参数；16/48 不冻结 |
| **CL-08** | 两层独立降级 | "没有任何候选达到阈值"而非"候选低置信"；允许单边合格 |
| **CL-10a** | 汇总默认打开+AI 自动分组 | `/nana/knowledge-map` 默认 Tab=题目汇总（**不是 /nana 首页**） |
| **CL-11** | 多题形成真正错题集 | 3 道跨章节题 → 3 个分组，章节标题正确 |
| **CL-12** | 琥珀"收过题"但不假绿 | 本次拍题不新增/更新 StudentNodeState；**已有绿色节点保留** |
| **CL-14** | 整体失败 vs 音频子失败拆清（🟡 A-1 补齐） | 整体失败→processingStatus=failed；音频失败→success+audioStatus=failed；**音频子失败无重试按钮，A-1 补齐**；重试不创重复 Case |
| **CL-15** | 连续拍题无竞态 | 3 题各自 AI 结果对应各自 caseId；旧请求不覆盖新状态 |
| **CL-16** | 用户间严格隔离 | 列表+直接接口越权（详情/process/tags 一律 404）；不泄漏 base64 |

### 9.2 未冻结条款列表（3 条）

| CL | 标题 | 未冻结原因 | 计划冻结时机 |
|----|------|------------|-------------|
| **CL-09** | 手动改分类（PATCH API + UI + 双写口径） | 依赖 TD-006 未解决 | Phase B / B-0 |
| **CL-10b** | 手动改分类后汇总以 manual 为权威 | 依赖 TD-006 未解决 | Phase B / B-0 |
| **CL-13** | 打印页 `/nana/print-preview` | 路由未实现 | Phase C / C-0 |

### 9.3 冻结纪律

- **冻结后**：A-1 起，所有 R1a 实现和测试必须严格遵守上述 14 条冻结要点
- **发现冻结条款需修改时**：必须回到 plan-agent 提出修订请求，说明事实依据，等待用户确认后才可解冻改条款；execute-agent 不得擅自修改冻结条款
- **A-1 补齐项的特别说明**：CL-06 的"暂未覆盖"占位、CL-14 的音频子失败重试按钮——这两处属于"已识别的部分实现，A-1 显式补齐"，不构成"违反冻结"——它们是冻结清单中**显式标注的待办**，不是事后发现的偏差
- **下次冻结**：Phase B 启动时执行 B-0（第二次冻结），冻结 CL-09 + CL-10b，更新本段为"已冻结条款清单（第二次）"
